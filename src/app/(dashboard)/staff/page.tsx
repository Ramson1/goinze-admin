...
export default function StaffPage() {
  ...
  async function handleApprovePortal(staffId: string, name: string) {
    setConfirmDialog({
      message: `Approve portal access for ${name}? This will activate their lecturer account.`,
      onConfirm: () => {
        setConfirmDialog(null);
        doApprovePortal(staffId);
      },
    });
  }
  ...
  async function doApprovePortal(staffId: string) {
    ...
    setNotice(null);
    try {
      await staffApi.approvePortal(staffId);
      setNotice('Lecturer portal account approved successfully.');
      loadPendingApprovals();
    } catch (err) {
  ...
  async function doApproveUnlinked(userId: string) {
    ...
    setNotice(null);
    try {
      await staffApi.approveUnlinkedUser(userId);
      setNotice('Lecturer account approved and staff record created successfully.');
      loadUnlinkedPending();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Approval failed.');
    } finally {
  ...
  return (
    <>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {pendingApprovals.length} lecturer account{pendingApprovals.length !== 1 ? 's' : ''} awaiting approval
                </p>
                <p className="text-xs text-gray-500">
                  These lecturers have registered for portal access and are waiting for admin approval.
                </p>
              </div>
            </div>
            <ChevronDown
      ...
      {/* Unlinked Portal Requests (no Staff record) */}
        ...
        <Card className="mb-6">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900">
                  {unlinkedPending.length} portal request{unlinkedPending.length !== 1 ? 's' : ''} without staff record
                </p>
                <p className="text-xs text-gray-500">
                  These users registered without a pre-existing staff record. Approving will create their staff record.
                </p>
              </div>
            </div>
            <ChevronDown
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {unlinkedPending.map((u) => {
                        const meta = u.metadata as { staffNumber?: string; departmentId?: string } | null;
                        return (
