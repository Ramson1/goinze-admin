...
export default function StudentsPage() {
  ...
  async function handleDeclinePortal(studentId: string, name: string) {
    setConfirmDialog({
      message: `Decline portal access for ${name}? This will delete their account request.`,
      onConfirm: () => {
        setConfirmDialog(null);
        doDeclinePortal(studentId);
      },
    });
  }
  ...
  async function doDeclinePortal(studentId: string) {
    setDecliningId(studentId);
    setError(null);
    setNotice(null);
    try {
      await studentsApi.declinePortal(studentId);
      setNotice('Portal request declined successfully.');
      loadPendingApprovals();
    } catch (err) {
  ...
  async function doDeclineUnlinked(userId: string) {
    ...
    setNotice(null);
    try {
      await studentsApi.declineUnlinkedUser(userId);
      setNotice('Registration request declined successfully.');
      loadUnlinkedPending();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Decline failed.');
    } finally {
